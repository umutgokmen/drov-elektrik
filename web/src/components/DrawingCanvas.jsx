...
const HDim = ({ x1, x2, y, value, below = true, offset = 22 }) => {
  ...
  return (
    <g className="dim dim-h">
      ...
      {/* Value */}
      <text x={mid} y={dy + 3.5} textAnchor="middle" fontSize="9" fontFamily={DIM_FONT} fill={DIM_COLOR} fontWeight="500">{value}</text>
    </g>
  );
};

const VDim = ({ y1, y2, x, value, right = true, offset = 22 }) => {
  ...
  return (
    <g className="dim dim-v">
      ...
      <text x={dx} y={mid} textAnchor="middle" fontSize="9" fontFamily={DIM_FONT} fill={DIM_COLOR} fontWeight="500"
        transform={`rotate(-90, ${dx}, ${mid})`}>{value}</text>
    </g>
  );
};
...
const HoleWithCenter = ({ cx, cy, r = 5, label }) => (
  <g>
    {/* Hole circle */}
    ...
    {label && (
      <text x={cx} y={cy - r - 5} textAnchor="middle" fontSize="6" fill="#64748b" fontFamily={DIM_FONT}>{label}</text>
    )}
  </g>
);
...
const SectionLine = ({ x1, y1, x2, y2, label }) => (
  <g>
    <line x1={x1} y1={y1} x2={x2} y2={y2}
      stroke="#1a1a1a" strokeWidth={0.7} strokeDasharray="12,3,2,3" />
    {/* Section labels with direction arrows */}
    {y1 === y2 ? (
      <>
        <polygon points={`${x1 - 2},${y1 - 8} ${x1 - 5},${y1 - 14} ${x1 + 1},${y1 - 14}`} fill="#1a1a1a" />
        <text x={x1 - 2} y={y1 - 16} textAnchor="middle" fontSize="11" fontWeight="700" fill="#1a1a1a" fontFamily={DIM_FONT}>{label}</text>
        <polygon points={`${x2 + 2},${y2 - 8} ${x2 - 1},${y2 - 14} ${x2 + 5},${y2 - 14}`} fill="#1a1a1a" />
        <text x={x2 + 2} y={y2 - 16} textAnchor="middle" fontSize="11" fontWeight="700" fill="#1a1a1a" fontFamily={DIM_FONT}>{label}</text>
      </>
    ) : (
      <>
        <polygon points={`${x1 + 8},${y1 - 2} ${x1 + 14},${y1 - 5} ${x1 + 14},${y1 + 1}`} fill="#1a1a1a" />
        <text x={x1 + 16} y={y1 + 4} textAnchor="start" fontSize="11" fontWeight="700" fill="#1a1a1a" fontFamily={DIM_FONT}>{label}</text>
        <polygon points={`${x2 + 8},${y2 + 2} ${x2 + 14},${y2 - 1} ${x2 + 14},${y2 + 5}`} fill="#1a1a1a" />
        <text x={x2 + 16} y={y2 + 4} textAnchor="start" fontSize="11" fontWeight="700" fill="#1a1a1a" fontFamily={DIM_FONT}>{label}</text>
      </>
    )}
  </g>
);
...
const TitleBlock = ({ x, y, boxName, boxId, date, scale = '1:2' }) => {
  const w = 180;
  const h = 65;
  return (
    <g>
...
const DrawingCanvas = ({ box, config }) => {
  ...
  // Compute scale to fit all 3 views
  const gapPx = 70;
  const availW = canvasWidth - 2 * drawingMargin - gapPx - 200; // 200 for title block area
    ...
    <svg width={canvasWidth} height={canvasHeight} viewBox={`0 0 ${canvasWidth} ${canvasHeight}`}
      ...
      <defs>
        ...
        <style>{`
          ...
          .phantom-line { stroke-dasharray: 12,3,2,3,2,3; }
        `}</style>
      </defs>

      {/* Background grid */}
      <rect x={borderMargin} y={borderMargin}
        width={canvasWidth - 2 * borderMargin} height={canvasHeight - 2 * borderMargin}
        fill="url(#grid-25)" />
      ...
      {/* Zone markers along borders */}
      {['A', 'B', 'C', 'D', 'E', 'F'].map((letter, i) => {
        const step = (canvasWidth - 2 * borderMargin) / 6;
        const xPos = borderMargin + i * step + step / 2;
        return (
          <g key={`zone-${letter}`}>
            <text x={xPos} y={borderMargin + 8} textAnchor="middle" fontSize="7" fill="#94a3b8" fontFamily={DIM_FONT}>{letter}</text>
            <text x={xPos} y={canvasHeight - borderMargin - 3} textAnchor="middle" fontSize="7" fill="#94a3b8" fontFamily={DIM_FONT}>{letter}</text>
          </g>
        );
      })}
      {[1, 2, 3, 4].map((num, i) => {
        ...
        return (
          <g key={`zone-${num}`}>
            <text x={borderMargin + 5} y={yPos + 3} fontSize="7" fill="#94a3b8" fontFamily={DIM_FONT}>{num}</text>
            <text x={canvasWidth - borderMargin - 5} y={yPos + 3} textAnchor="end" fontSize="7" fill="#94a3b8" fontFamily={DIM_FONT}>{num}</text>
          </g>
        );
      })}
      ...
      <g>
        <text x={frontX + fW / 2} y={frontY - 14} textAnchor="middle" fontSize="10" fontWeight="700"
          fontFamily={DIM_FONT} fill="#0f172a" letterSpacing="0.5">FRONT VIEW</text>

        {/* Outer wall - thick line */}
        <rect x={frontX} y={frontY} width={fW} height={fH}
          fill="#fafcff" stroke="#0f172a" strokeWidth={1.4} />
        ...
        <rect x={frontX} y={frontY + fH - wallPx} width={fW} height={wallPx} fill="url(#hatch)" stroke="none" />

        {/* Inner cavity - thin line */}
        <rect x={frontX + wallPx} y={frontY + wallPx}
          width={fW - 2 * wallPx} height={fH - 2 * wallPx}
          fill="none" stroke="#475569" strokeWidth={0.6} />

        {/* Center lines */}
        <line x1={frontX - 8} y1={frontY + fH / 2} x2={frontX + fW + 8} y2={frontY + fH / 2}
          stroke="#94a3b8" strokeWidth={THIN} className="center-line" />
        <line x1={frontX + fW / 2} y1={frontY - 8} x2={frontX + fW / 2} y2={frontY + fH + 8}
          stroke="#94a3b8" strokeWidth={THIN} className="center-line" />

        {/* Top holes (shown at bottom of front view in 3rd angle) */}
        {holesTop.map((pos, i) => (
          <HoleWithCenter key={`ft-${i}`} cx={frontX + pos * s} cy={frontY + fH} r={holeR}
            label={config.holeSizeTop || 'M20'} />
        ))}
        {/* Bottom holes (shown at top of front view) */}
        {holesBottom.map((pos, i) => (
          <HoleWithCenter key={`fb-${i}`} cx={frontX + pos * s} cy={frontY} r={holeR}
            label={config.holeSizeBottom || 'M20'} />
        ))}
      ...
      {/* ================ TOP VIEW (Width × Length) ================ */}
      <g>
        <text x={topX + tW / 2} y={topY + tH + 20} textAnchor="middle" fontSize="10" fontWeight="700"
          fontFamily={DIM_FONT} fill="#0f172a" letterSpacing="0.5">TOP VIEW (PLAN)</text>
        ...
        {/* Inner cavity */}
        <rect x={topX + wallPx} y={topY + wallPx}
          width={tW - 2 * wallPx} height={tH - 2 * wallPx}
          fill="none" stroke="#475569" strokeWidth={0.6} />

        {/* Center lines */}
        <line x1={topX - 8} y1={topY + tH / 2} x2={topX + tW + 8} y2={topY + tH / 2}
          stroke="#94a3b8" strokeWidth={THIN} className="center-line" />
        <line x1={topX + tW / 2} y1={topY - 8} x2={topX + tW / 2} y2={topY + tH + 8}
          stroke="#94a3b8" strokeWidth={THIN} className="center-line" />

        {/* DIN Rails with terminal blocks */}
        {Array.from({ length: railCount }).map((_, i) => {
          const railY = railCount > 1
            ? topY + (railMargin + i * railSpacing) * s
            : topY + tH / 2;
          const railW = tW - 2 * (25 * s);
          const railX = topX + 25 * s;
          const railH = 7 * s;
          const termCount = Math.min(terminalsPerRail, config.terminals - i * terminalsPerRail);

          return (
            <g key={`rail-${i}`}>
              {/* DIN Rail - hatched rectangle */}
              <rect x={railX} y={railY - railH / 2} width={railW} height={railH}
                fill="#f1f5f9" stroke="#64748b" strokeWidth={0.7} />
              {/* Rail center line */}
              <line x1={railX} y1={railY} x2={railX + railW} y2={railY}
                stroke="#94a3b8" strokeWidth={THIN} className="center-line" />
              {/* Rail label */}
              ...
              {Array.from({ length: Math.max(0, termCount) }).map((_, t) => {
                ...
                if (tx + termW > railX + railW - 4 * s) return null;
                return (
                  <g key={`t-${i}-${t}`}>
                    <rect x={tx} y={railY - termH / 2} width={termW} height={termH}
                      fill="#e2e8f0" stroke="#475569" strokeWidth={0.4} rx={0.5} />
                    {/* Terminal screw dot */}
        ...
        {/* Left holes */}
        {holesLeft.map((pos, i) => (
          <HoleWithCenter key={`tl-${i}`} cx={topX} cy={topY + pos * s} r={holeR}
            label={config.holeSizeLeft || 'M20'} />
        ))}
        {/* Right holes */}
        {holesRight.map((pos, i) => (
          <HoleWithCenter key={`tr-${i}`} cx={topX + tW} cy={topY + pos * s} r={holeR}
            label={config.holeSizeRight || 'M20'} />
        ))}

        {/* Section line B-B (vertical) */}
      ...
      {/* ================ RIGHT VIEW (Length × Depth) ================ */}
      <g>
        <text x={rightX + rW / 2} y={rightY - 14} textAnchor="middle" fontSize="10" fontWeight="700"
          fontFamily={DIM_FONT} fill="#0f172a" letterSpacing="0.5">RIGHT VIEW</text>
        ...
        {/* Inner cavity */}
        <rect x={rightX + wallPx} y={rightY + wallPx}
          width={rW - 2 * wallPx} height={rH - 2 * wallPx}
          fill="none" stroke="#475569" strokeWidth={0.6} />

        {/* Center lines */}
        <line x1={rightX - 8} y1={rightY + rH / 2} x2={rightX + rW + 8} y2={rightY + rH / 2}
          stroke="#94a3b8" strokeWidth={THIN} className="center-line" />
        <line x1={rightX + rW / 2} y1={rightY - 8} x2={rightX + rW / 2} y2={rightY + rH + 8}
          stroke="#94a3b8" strokeWidth={THIN} className="center-line" />

        {/* Right/Left holes projected onto side view */}
        {holesRight.map((pos, i) => (
        ...
        {Array.from({ length: railCount }).map((_, i) => {
          const railYNorm = railCount > 1
            ? railMargin + i * railSpacing
            : L / 2;
          const railYPx = rightY + railYNorm * s * (rH / tH);
          return (
            <line key={`rrl-${i}`}
              x1={rightX + wallPx + 2} y1={railYPx}
              x2={rightX + rW - wallPx - 2} y2={railYPx}
              stroke="#94a3b8" strokeWidth={0.5} className="hidden-line" />
          );
        })}
      ...
      {/* ================ PROJECTION LINES (thin dotted) ================ */}
      <g opacity={0.25}>
        {/* Front to Top alignment lines */}
        <line x1={frontX} y1={frontY + fH + 5} x2={topX} y2={topY - 5}
          stroke="#94a3b8" strokeWidth={THIN} strokeDasharray="2,4" />
        <line x1={frontX + fW} y1={frontY + fH + 5} x2={topX + tW} y2={topY - 5}
          stroke="#94a3b8" strokeWidth={THIN} strokeDasharray="2,4" />
        {/* Front to Right alignment lines */}
        <line x1={frontX + fW + 5} y1={frontY} x2={rightX - 5} y2={rightY}
          stroke="#94a3b8" strokeWidth={THIN} strokeDasharray="2,4" />
        <line x1={frontX + fW + 5} y1={frontY + fH} x2={rightX - 5} y2={rightY + rH}
          stroke="#94a3b8" strokeWidth={THIN} strokeDasharray="2,4" />
      </g>
      ...
      <TitleBlock
        x={canvasWidth - 200}
        y={canvasHeight - borderMargin - 75}
        boxName={box.name}
        boxId={box.id}
        date={today}
      />
