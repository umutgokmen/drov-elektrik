...
def validate_hole_placement(
    ...
    Supports multiple hole sizes (M20-M50) and series-specific rules.
    """
    if hole_count == 0:
        return True, "", 0
    ...
    if box_id:
        clearance = max(clearance, _get_hole_clearance_for_box(box_id))
        edge_margin = _get_edge_margin_for_box(box_id)
    else:
        edge_margin = MIN_EDGE_MARGIN
...
def validate_terminal_placement(terminal_count: int, box: BoxModel) -> Tuple[bool, str, int]:
    ...
    if terminal_count > max_total:
        return (
            False,
            f"Ray kapasitesi asildi. Fiziksel maksimum: {max_total} klemens.",
            max_total
        )
    ...
    if terminal_count > box.max_terminals:
        return (
            False,
            f"Kutu kapasitesi asildi. Maksimum: {box.max_terminals} klemens.",
            box.max_terminals
        )
...
def run_full_validation(config: ConfigurationInput) -> ValidationResult:
    ...
    Supports EJB, EJC, EJBX, ESP, ESA, and ESX box types.
    """
    ...
    if not box:
        return ValidationResult(
            is_valid=False,
            errors=[ValidationError(field="box_id", message=f"Gecersiz kutu modeli: {config.box_id}")],
            warnings=[]
        )

    # Hole validations with per-side hole size support
    hole_validations = [
    ...
    for field, count, length, hole_size in hole_validations:
        is_valid, message, max_possible = validate_hole_placement(
            count, length, config.box_id, hole_size
        )
        if not is_valid:
    ...
    if config.terminals > box.max_terminals * 0.9 and term_valid:
        warnings.append(ValidationWarning(
            field="terminals",
            message="Klemens kapasitesinin %90'ina yaklastiniz."
        ))
    ...
    if max_total_holes > 0 and total_holes > max_total_holes * 0.9:
        warnings.append(ValidationWarning(
            field="holes",
            message="Toplam delik kapasitesinin %90'ina yaklastiniz."
        ))
